...
rg -Fq 'body.theme-light .weekly-voyage .cx-weekly-analytics {' "$plugin/styles.css" || fail "Weekly Analytics missing explicit light-mode surface"
...
rg -Fq 'appendFile(target' "$plugin/main.js" || fail "LeetCode Tracker does not append completed Study Blocks"
rg -q 'leetcodeTimers' "$plugin/main.js" || fail "LeetCode Tracker timer state is not persisted"
rg -Fq 'event_type: "study_block_completed"' "$plugin/main.js" || fail "LeetCode Tracker missing schema-v4 Study Block output"
rg -Fq 'duration_source: "timer"' "$plugin/main.js" || fail "LeetCode Tracker missing timer duration source"
rg -Fq 'duration_source: "manual"' "$plugin/main.js" || fail "LeetCode Tracker missing manual duration fallback"
rg -Fq 'const TRACKER_OUTCOMES = [' "$plugin/main.js" || fail "LeetCode Tracker missing problem outcomes"
for outcome in attempted partial completed reviewed; do
  rg -Fq "{ id: \"${outcome}\"" "$plugin/main.js" || fail "LeetCode Tracker missing ${outcome} outcome"
  rg -Fq "body.theme-light .cx-lc-status.is-${outcome}" "$plugin/styles.css" || fail "Light-mode LeetCode ${outcome} status lacks semantic contrast"
done
rg -Fq 'Start Study Block' "$plugin/main.js" || fail "LeetCode Tracker missing Study Block start action"
rg -Fq 'Finish Study Block' "$plugin/main.js" || fail "LeetCode Tracker missing Study Block finish action"
if rg -q 'TRACKER_PHASES|phase_tracking|phase_seconds|expected_active_minutes|Start Session|Finish · Completed' "$plugin/main.js"; then
  fail "Round 1 Tracker still exposes legacy per-problem or phase timing"
fi
rg -Fq 'must never be divided, copied, or inferred' "$schema" || fail "Schema does not forbid per-problem duration inference"
rg -Fq 'mixed log' "$schema" || fail "Schema does not define mixed legacy/v4 event reading"
...
rg -Fq 'body.theme-light .cx-lc-block-prompt { color: #315f78; }' "$plugin/styles.css" || fail "Light-mode LeetCode recommendation text lacks readable contrast"
rg -Fq 'body.theme-light .cx-lc-block-status { color: #315f78; }' "$plugin/styles.css" || fail "Light-mode LeetCode Study Block status lacks readable contrast"
rg -Fq 'body.theme-light .cx-lc-manual-duration label { color: #456b82; }' "$plugin/styles.css" || fail "Light-mode LeetCode manual duration label lacks readable contrast"
...
pass "embedded desktop Study Block tracker and mixed Bridge event boundary"
...
rg -Fq 'function weeklyDayMetricsEligible(frontmatter, startValue, endValue)' "$plugin/main.js" || fail "Weekly history missing period-level day-metrics eligibility"
rg -Fq 'String(frontmatter?.type || "").trim() === "weekly-review"' "$plugin/main.js" || fail "Weekly history does not validate the Weekly type"
rg -Fq 'isoDateValue(frontmatter?.period_start) === start' "$plugin/main.js" || fail "Weekly history does not validate period_start"
rg -Fq 'isoDateValue(frontmatter?.period_end) === end' "$plugin/main.js" || fail "Weekly history does not validate period_end"
rg -Fq 'String(frontmatter?.day_metrics_model || "").trim() === DAY_METRICS_MODEL' "$plugin/main.js" || fail "Weekly history does not validate its own model marker"
rg -Fq 'weekDays.every((day) => day.file instanceof TFile)' "$plugin/main.js" || fail "Weekly history does not require seven canonical Daily files"
rg -Fq '} else if (week.dayMetricsEnabled) {' "$plugin/main.js" || fail "Weekly trend still limits day metrics to the current period"
rg -Fq 'this.app.vault.on("create", schedule)' "$plugin/main.js" || fail "Weekly history does not refresh when a missing source is created"
rg -Fq 'this.app.vault.on("delete", schedule)' "$plugin/main.js" || fail "Weekly history does not refresh when a source is deleted"
rg -Fq 'this.app.vault.on("rename", schedule)' "$plugin/main.js" || fail "Weekly history does not refresh when a source is renamed"
...
rg -Fq '"Morning Check-in"' "$plugin/main.js" || fail "Weekly Analytics mislabels the morning timestamp"
rg -Fq 'Project Activity Mode' "$plugin/main.js" || fail "Weekly Analytics missing Activity Mode allocation"
rg -Fq 'Mental Terms' "$plugin/main.js" || fail "Weekly Analytics missing Mental term frequencies"
rg -Fq 'Project Velocity' "$plugin/main.js" || fail "Weekly Analytics missing weighted Project velocity"
rg -Fq 'Health × Mental × Time' "$plugin/main.js" || fail "Weekly Analytics missing cross-domain alignment"
rg -Fq 'Workout Decision → Execution' "$plugin/main.js" || fail "Weekly Analytics missing workout decision funnel"
rg -Fq 'Week-over-Week Investment Change' "$plugin/main.js" || fail "Weekly Analytics missing rolling investment change"
rg -Fq 'each visible week compared with its preceding week' "$plugin/main.js" || fail "Weekly investment chart missing per-column comparison semantics"
rg -Fq 'historyWeeks * 7' "$plugin/main.js" || fail "Weekly investment chart does not load the hidden preceding week"
rg -Fq 'previousWeek?.days.length === 7 && previousWeek.days.every' "$plugin/main.js" || fail "Weekly investment comparison does not reject partial preceding weeks"
rg -Fq 'Capacity–Load Gap' "$plugin/main.js" || fail "Weekly Analytics missing Capacity-Load Gap"
rg -Fq 'Core Execution Ratio' "$plugin/main.js" || fail "Weekly Analytics missing Core Execution Ratio"
rg -Fq 'Project Velocity' "$plugin/main.js" || fail "Weekly Analytics missing Project Velocity"
rg -Fq 'Time Fragmentation Index' "$plugin/main.js" || fail "Weekly Analytics missing Time Fragmentation Index"
rg -Fq 'coreExecutionMinutes' "$plugin/main.js" || fail "Weekly Analytics missing frozen-Core Execution numerator"
rg -Fq -- '--cx-weekly-gap-over:' "$plugin/styles.css" || fail "Weekly Capacity-Load Gap missing dedicated over-capacity color"
rg -Fq -- '--cx-weekly-gap-reserve:' "$plugin/styles.css" || fail "Weekly Capacity-Load Gap missing dedicated reserve color"
rg -Fq -- '--cx-weekly-project: #79b83d;' "$plugin/styles.css" || fail "Light Weekly Project missing Odd Garden lime"
rg -Fq -- '--cx-weekly-enrichment: #25a7b8;' "$plugin/styles.css" || fail "Light Weekly Enrichment missing Odd Garden teal"
rg -Fq -- '--cx-weekly-workout: #f06f5d;' "$plugin/styles.css" || fail "Light Weekly Workout missing Odd Garden coral"
rg -Fq -- '--cx-weekly-admin: #7668c8;' "$plugin/styles.css" || fail "Light Weekly Admin missing Odd Garden violet"
rg -Fq -- '--cx-weekly-health-morning: #62bdd0;' "$plugin/styles.css" || fail "Light Weekly Health Capacity missing Odd Garden aqua"
rg -Fq 'body.theme-light .weekly-voyage .cx-weekly-frequency-dots { color: #62bdd0; }' "$plugin/styles.css" || fail "Light Weekly term dots missing Odd Garden aqua"
rg -Fq 'background-color: #edf6f8 !important;' "$plugin/styles.css" || fail "Light Weekly page missing requested background"
rg -Fq 'background: #f8fcfd !important;' "$plugin/styles.css" || fail "Light Weekly cards missing requested background"
rg -Fq '.cx-weekly-investment-delta { border: 1px solid currentColor; border-radius: 7px;' "$plugin/styles.css" || fail "Weekly investment deltas missing rounded borders"
rg -Fq 'const edgeInset = 18;' "$plugin/main.js" || fail "Weekly SVG charts missing inner plot inset"
rg -Fq 'body.theme-light .weekly-voyage .cx-weekly-cross-row.is-time span:not(:first-child)' "$plugin/styles.css" || fail "Weekly cross-domain Time row missing a distinct background"
rg -Fq 'WeeklySnapshotChild.prototype.renderStateChart.call(this, wrap, days);' "$plugin/main.js" || fail "Weekly state relationship was not moved into Detailed Analytics"
rg -Fq 'this.renderInvestmentChange(wrap, weeks, precedingWeek);' "$plugin/main.js" || fail "Weekly analytics missing outcome-led investment ordering"
if rg -Fq 'backdrop-filter: blur(14px) saturate(112%)' "$plugin/styles.css"; then
  fail "Light Weekly surfaces must not use the reverted glass blur"
fi
if rg -Fq '.cx-weekly-gap-fill.is-over { background: var(--cx-weekly-workout); }' "$plugin/styles.css" || rg -Fq '.cx-weekly-gap-fill.is-reserve { background: var(--cx-weekly-project); }' "$plugin/styles.css"; then
  fail "Weekly Capacity-Load Gap must not reuse Workout or Project category colors"
fi
rg -Fq '✅ ?(\d{4}-\d{2}-\d{2})' "$plugin/main.js" || fail "Weekly Project progress is not anchored to dated task completion"
...
pass "Weekly Snapshot, detailed analytics, source period, and read-only derived data"
